document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (tableBody) {
        async function loadInvoices() {
            try {
                const response = await fetch('http://localhost:3000/api/payments/invoices', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
        ...
        window.payInvoice = async (invoiceId) => {
            ...
            try {
                const response = await fetch(`http://localhost:3000/api/payments/pay/${invoiceId}`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    if (servicesContainer) {
        async function loadActiveServices() {
            try {
                const response = await fetch('http://localhost:3000/api/payments/services', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
