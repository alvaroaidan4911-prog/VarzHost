document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (statActive || statUnpaid || statTotal) {
        async function loadDashboardStats() {
            try {
                const response = await fetch('https://smoke-sells-spyware-include.trycloudflare.com/api/payments/stats', {
    ...
    if (tableBody) {
        async function loadInvoices() {
            try {
                const response = await fetch('https://smoke-sells-spyware-include.trycloudflare.com/api/payments/invoices', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
        ...
        window.payInvoice = async (invoiceId) => {
            ...
            try {
                const response = await fetch(`https://smoke-sells-spyware-include.trycloudflare.com/api/payments/pay/${invoiceId}`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    if (servicesContainer) {
        async function loadActiveServices() {
            try {
                const response = await fetch('https://smoke-sells-spyware-include.trycloudflare.com/api/payments/services', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
